...
onSubmitHandler(event) {
    ...
    if (this.dTWhistList.isAddedIntoList(this.productHandle)) {
        ...
        this.addWishList.classList.add("removing");
        this.dTWhistList.removeWhishlist(this.productHandle);
        setTimeout(this.postRemove.bind(this), 500);
    } else {
        // Add to wishlist
...
// Add this new method for removing
postRemove() {
    var dtxwishCount = document.getElementsByClassName('dtxc-wishlist-count');
    if (dtxwishCount.length > 0) {
        document.querySelector(".dtxc-wishlist-count").setAttribute(
    ...
    this.addWishList.classList.remove("removing");
    this.addWishList.classList.remove("added");   
}
